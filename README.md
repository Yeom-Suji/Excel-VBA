...
😀 데이터를 의심하고 검증하라(데이터가 분석하기에 바람직 한가?)<br/>
...
2. 일반 데이터 범위인지 표개체인지 확인하기<br/>
	...
	표개체를 일반데이터 범위로 바꾸는 방법 [표도구 -> 일반데이터변환]<br/>
		1. 표개체<br/>
			1. 동적 범위로 자동으로 잡아준다<br/>
			2. 필드 단위 계산: 구조적 참조<br/>
...
4. 필드별 데이터 타입이 정확하게 잡혀있는지<br/>
	숫자 문자 날짜 수식 中 하나<br/>
<br/><br/>
😀 파워쿼리로 가공하는 표준 프로세스<br/>
일반데이터범위 -> 표체게 -> 파워쿼리로 불러가서 가공(텍스트로 바꾸고 날짜로 바꾸면 히스토리에 남길 수 있음) -> 위크시트로 가공
-> 원본 업뎃 -> 쿼리내에서 새로고침 -> 검증(원본의 데이터가 더 들어왔는지 새로고침해서 확인)<br/>
동료가 연결한 파워쿼리를 확인하려면? 데이터 -> 쿼리 및 연결<br/>
뒤에 필드를 더 추가하고 싶을 때? 리본메뉴에서 열추가 -> 사용자 지정열 -> 사용자 지정 열 수식 정해주기(ex. ="YSJ") -> 문법적으로 맞는지 확인<br/> 
결측치 제거하고 싶을 때? 파워쿼리창에서 마지막 열 아래 아이콘 -> 결측치나 네임error 없앰<br/>
<br/><br/>
😀 원본 데이터 설정 (원본 폴더나 원본 파일의 경로가 바뀌거나 이름이 바뀌면?)<br/>
